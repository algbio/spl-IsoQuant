...
Takes a configuration file as input, runs splisoquant_detect_barcodes.py on simulated data,
...
def parse_args():
    ...
    parser.add_argument("--additional_options", "-a", type=str,
                        help="Additional options for splisoquant_detect_barcodes.py")
...
def run_detect_barcodes(args, config_dict):
    """Run splisoquant_detect_barcodes.py with configuration options."""
    source_dir = os.path.dirname(os.path.realpath(__file__))
    isoquant_dir = os.path.join(source_dir, "../../")
    config_file = args.config_file

    run_name = config_dict["name"]
    # splisoquant_detect_barcodes.py uses -o as a file prefix, not a directory
    ...
    log.info('== Running splisoquant_detect_barcodes.py ==')

    # Build command
    detect_command = [
        "python3", os.path.join(isoquant_dir, "splisoquant_detect_barcodes.py"),
        "-o", output_prefix,
    ...
    if result.returncode != 0:
        log.error("splisoquant_detect_barcodes.py exited with non-zero status: %d" % result.returncode)
        return -11
...
def run_barcode_quality(args, config_dict):
    ...
    # splisoquant_detect_barcodes.py outputs: {prefix}.barcoded_reads.tsv
